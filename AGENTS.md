...
- **`llama-server` is Ollama's child process.** Killing it is futile; `ollama serve` respawns it. Stop Ollama itself. See the global `~/.Codex/AGENTS.md` traps section.
...
- `~/Library/Application Support/com.prakashjoshipax.VoiceInk/WhisperModels/ggml-large-v3*` — the Hinglish model and its encoder. It looks like a duplicate of a turbo model. It is not; it is the only thing covering Hindi-English code-switching. Full reasoning is in the global `~/.Codex/AGENTS.md` traps section.
